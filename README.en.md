...
> This repository evolves from [xmanrui/dsh-im](https://github.com/xmanrui/dsh-im) and is published as `@tokensapi/dsh-connect`. The original authorship, contribution history, and MIT license are preserved. The current package combines IM bots with personal Feishu and DingTalk authorization in one TokensHarness Connection Center.

## Introduction

Connect IM bots to DeepSeek Harness by scanning a QR code, using an App Manifest, or entering existing bot credentials, and authorize personal Feishu or DingTalk accounts through OAuth. One plugin and one Connection Center entry separate ten multi-bot IM channels from two personal account connections.
...
Personal DingTalk authorization uses managed OAuth from the official [DingTalk Workspace CLI](https://github.com/DingTalk-Real-AI/dingtalk-workspace-cli) (DWS). Connection Center returns a complete `login.dingtalk.com` link with the authorization code embedded, so no AppKey or AppSecret is required. After authorization, the plugin installs a checksum-verified DWS binary and official `dingtalk-*` Skills. DingTalk IM bots remain an independent Stream channel with separate credentials and lifecycle.
...
| QQ | Create a bot in QQ Open Platform, then bind it with AppID + AppSecret | WebSocket connection; private chats show typing and receive one Markdown reply, while mentioned group chats receive only the final answer |
...
| iMessage | Grant local Messages.app permissions on macOS | Native macOS Messages text send and receive; reads the local message database and replies through AppleScript |

Other IM platforms can be added through the same channel-adapter structure.

The nine built-in channels with media support can send JPEG, PNG, and WebP images, plus GIFs sent as image files, with optional captions to Harness. Each image is limited to 5 MB, and images in one message are limited to 20 MB in total. iMessage currently supports text only. Downloading images or files from Feishu user messages requires the `im:message:readonly` tenant scope, shown on the confirmation page as **Read direct and group messages**; Feishu currently offers no narrower image-only scope for that download endpoint. Apps created through the built-in QR flow request it by default; for existing or manually connected apps, click **Complete permissions** in the plugin's Feishu settings and scan the QR code to incrementally add that scope, `im:resource` for uploading bot-sent images or files, and the card callback.
...
After installation, follow the built-in instructions on each channel page to scan a QR code, enter credentials, or authorize a personal account. Personal DingTalk authorization uses the one-click link generated by Connection Center; on first use it downloads and verifies DWS and its Skills from the official GitHub Release. Secrets and Tokens are sent only to the local Harness Host or written by DWS to protected local storage; status responses and bot lists never return them. The upgrade installer removes the legacy `@tokens/dsh-connect`, `@tokens/dsh-im`, `@tokens/dsh-feishu-connect`, and `@tokens/dsh-connect-ui` packages while preserving bot bindings and the existing `dsh-feishu` authorization data.
...
- Registers one **Connection Center** settings page with separate **IM bots** and **App authorization** tabs for ten IM channels plus personal Feishu and DingTalk connections.
- Maintains the Host, client, and runtime sources for all ten channels and both personal connections in this repository without external standalone plugins.
...
- Reuses the original dsh-im logos and card layout for WeChat, Feishu Bot, DingTalk, WeCom, QQ, Slack, Telegram, Discord, WhatsApp, iMessage, Personal Feishu, and Personal DingTalk.
- Keeps RPC endpoints, credentials, connection supervision, and session mappings isolated by IM channel. Personal Feishu retains the `dsh-feishu` profile. Personal DingTalk keeps an isolated DWS configuration inside the current DSH profile and exposes official Skill commands through one restricted `dws` tool.
...
`rpcAuthority` defaults to `trusted-host`; explicitly selecting `loopback` additionally requires loopback Host and Origin values. `trusted-host` reuses Harness's Host/Origin fence; it is not user authentication. Anyone who can reach that LAN authority can inspect bot status, scan or submit application credentials, reconnect bots, and remove bots, so use it only on a trusted network.
