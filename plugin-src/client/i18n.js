...
const EN = Object.freeze({
  ...
  "${skills.count} 个官方 Skills · DWS ${skills.version ?? status?.version ?? ''}": "${skills.count} official Skills · DWS ${skills.version ?? status?.version ?? ''}",
  ...
  "Token ${personalIdentity.tokenStatus ?? 'valid'} · 服务端验证通过": "Token ${personalIdentity.tokenStatus ?? 'valid'} · server verified",
  ...
  "${application.name} · ${application.appIdMasked}${application.botCount > 0 ? ' · 与机器人共用' : ''}": "${application.name} · ${application.appIdMasked}${application.botCount > 0 ? ' · Shared with bot' : ''}",
  ...
  '${application.name} · ${application.appIdMasked}${application.usedByPersonal ? " · 个人授权共用" : ""}': '${application.name} · ${application.appIdMasked}${application.usedByPersonal ? " · Shared with personal authorization" : ""}',
...
function translateDynamic(text) {
  ...
  match = /^Token (.+) · 服务端验证通过$/.exec(text);
  if (match) return `Token ${match[1]} · server verified`;
