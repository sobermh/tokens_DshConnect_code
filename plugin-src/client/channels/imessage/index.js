...
function PermissionPanel({ permissions, busy, error, onSubmit, onCancel }) {
  const unsupported = permissions?.database === 'unsupported'
    || permissions?.automation === 'unsupported';
  ...
  if (unsupported) {
    ...
    h('h3', null, 'macOS 原生 iMessage 暂不可用'),
    h('p', null, 'iMessage 连接依赖 macOS Messages.app，只能在 macOS 上启用。'),
    h('div', { className: 'ddt-actions dim-viewActions' },
      h('button', { type: 'button', className: 'ddt-button', onClick: onCancel }, '关闭')));
  }
