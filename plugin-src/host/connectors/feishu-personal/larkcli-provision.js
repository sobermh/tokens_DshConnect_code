...
async function httpBytes(url, maxBytes) {
    ...
    if (initial.protocol !== 'https:' || !GITHUB_HOSTS.includes(initial.hostname)) {
    ...
    if (!response.ok)
        throw new Error(`lark-cli: fetch failed (HTTP ${response.status})`);
    try {
        const finalHost = new URL(response.url).hostname;
        if (finalHost !== '' && !GITHUB_HOSTS.includes(finalHost)) {
    ...
    if (bytes.byteLength > maxBytes)
        throw new Error('lark-cli: fetched resource is too large');
...
async function fetchLatest() {
    ...
    if (!/^v?\d+\.\d+\.\d+/.test(rawTag))
        throw new Error('lark-cli: could not resolve latest release tag');
    ...
    if (Array.isArray(data.assets)) {
        for (const asset of data.assets) {
            ...
            if (typeof name === 'string' && typeof dl === 'string')
                assets.set(name, dl);
        }
    }
...
function resolveLatest() {
    if (latestMemo === undefined) {
        latestMemo = fetchLatest().catch((error) => {
...
function checksumFor(manifest, name) {
    for (const line of manifest.split(/\r?\n/)) {
        ...
        if (match !== null && match[2].trim() === name)
...
function extractZipEntry(zip, entryName) {
    ...
    if (eocd < 0)
        throw new Error('lark-cli: zip end-of-central-directory not found');
    const count = zip.readUInt16LE(eocd + 10);
    let p = zip.readUInt32LE(eocd + 16);
    for (let n = 0; n < count; n++) {
        if (zip.readUInt32LE(p) !== ZIP_CD_SIG)
            throw new Error('lark-cli: corrupt zip central directory');
        ...
        if (name === entryName) {
            if (uncompSize > MAX_BINARY_SIZE)
                throw new Error('lark-cli: zip entry is too large');
            if (zip.readUInt32LE(localOff) !== ZIP_LOCAL_SIG)
                throw new Error('lark-cli: corrupt zip local header');
            ...
            if (out.length !== uncompSize)
                throw new Error('lark-cli: zip entry size mismatch after inflate');
            return out;
        }
...
function extractTarGzEntry(archive, entryName) {
    ...
    while (off + 512 <= tar.length) {
        ...
        if (name === '')
            break; // zero block terminates the archive
...
let ensureInFlight;
async function install(info, release, target) {
    const name = assetName(info, release.tag);
    const assetUrl = release.assets.get(name);
    if (assetUrl === undefined)
        throw new Error(`lark-cli: release ${release.tag} has no asset ${name}`);
    const sumsUrl = release.assets.get('checksums.txt');
    if (sumsUrl === undefined)
        throw new Error(`lark-cli: release ${release.tag} has no checksums.txt`);
    ...
    if (sha256(archive) !== expected)
        throw new Error('lark-cli: downloaded archive checksum mismatch');
    ...
    if (binary.length > MAX_BINARY_SIZE)
        throw new Error('lark-cli: extracted binary is too large');
...
export function ensureLarkcli() {
    if (ensureInFlight === undefined) {
        ensureInFlight = doEnsure().catch((error) => {
 ...
 */
export async function larkcliPath() {
    let info;
    try {
        info = platformInfo();
    }
 ...
 */
export function installedLarkVersion() {
    return readStamp();
}
/**
 ...
 */
export function probeLarkcli(bin) {
