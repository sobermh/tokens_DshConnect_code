...
export function assertLarkProfileName(profile) {
...
export function identityFile(profile, root = dir()) {
 ...
 */
export async function readIdentity(profile) {
    try {
        return JSON.parse(await readFile(identityFile(profile), 'utf8'));
    }
    catch {
        return undefined;
    }
 ...
 */
export async function writeIdentity(record) {
 ...
 * @param profile - lark-cli profile name.
 */
export async function clearIdentity(profile) {
