...
export class IMessageRuntime {
  ...
  #pollIntervalMs;
  #createApi;
  #status = createIMessageRuntimeStatus();
  #api;
  #bridge;
  #abortController;
  #timer;
  ...
  constructor({
    ...
    if (!config || !token || !harness || !state) {
      throw new TypeError('IMessageRuntime requires config, token, Harness, and state');
    }
    this.#config = config;
  ...
  async sendConnectionTest(text) {
    if (!this.#status.ready || !this.#api) {
      const error = new Error('iMessage gateway is not connected');
  ...
  async sendProactiveText(target, text, options = {}) {
    ...
    if (!chatGuid) {
      const error = new TypeError('Invalid iMessage proactive delivery target');
  ...
  async start() {
    ...
    try {
      ...
      this.#bridge = new IMessageHarnessBridge({
        bot: client,
  ...
  async #poll() {
    ...
    try {
      const rows = await this.#api.listMessages({
        after: this.#state.cursor() ?? 0,
        limit: 100,
      });
      ...
      if (!this.#stopped) {
        this.#status.lastError = error.message;
        this.#logger.warn?.('[dsh-im:imessage] polling failed', error);
      }
    }
  }
}
