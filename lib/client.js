    ...
    return {
      ...
      health: {
        summary: text(
          ...
          connected2 ? `${channel4}${connectionSummary}\u8FD0\u884C\u6B63\u5E38` : `${channel4}\u8FDE\u63A5\u5C1A\u672A\u5C31\u7EEA`
        ),
...
function normalizeBot3(value) {
  ...
  return {
    ...
    health: {
      summary: text2(value.health?.summary, connected2 ? "QQ WebSocket \u957F\u8FDE\u63A5\u8FD0\u884C\u6B63\u5E38" : "QQ \u8FDE\u63A5\u5C1A\u672A\u5C31\u7EEA"),
...
function normalizeBot4(value) {
  ...
  return {
    ...
    health: {
      summary: text3(value.health?.summary, connected2 ? "\u4F01\u4E1A\u5FAE\u4FE1 WebSocket \u957F\u8FDE\u63A5\u8FD0\u884C\u6B63\u5E38" : "\u4F01\u4E1A\u5FAE\u4FE1\u8FDE\u63A5\u5C1A\u672A\u5C31\u7EEA"),
...
function normalizeBot6(value) {
  ...
  return {
    ...
    health: {
      summary: text4(value.health?.summary, connected2 ? "WhatsApp Web \u5173\u8054\u8BBE\u5907\u8FD0\u884C\u6B63\u5E38" : "WhatsApp \u8FDE\u63A5\u5C1A\u672A\u5C31\u7EEA"),
...
function DingtalkPersonalSettings({ rpcCall }) {
  ...
  const [status, setStatus] = React19.useState(initial);
  const [busy, setBusy] = React19.useState(initial === void 0 ? "load" : void 0);
  const [error, setError] = React19.useState();
  const [tab, setTab] = React19.useState("connection");
  const commit = React19.useCallback((next) => {
    cacheDingtalkPersonalStatus(rpcCall, next);
    setStatus(next);
  }, [rpcCall]);
  const load = React19.useCallback(async (quiet = false, refresh = true) => {
  ...
  const connect = React19.useCallback(async (force) => {
    setBusy("connect");
    try {
...
function FeishuPersonalSettings({ rpcCall }) {
  ...
  const [status, setStatus] = React20.useState(initialStatus);
  const [error, setError] = React20.useState();
  const [busy, setBusy] = React20.useState(initialStatus === void 0 ? "load" : void 0);
  const [domain, setDomain] = React20.useState("feishu");
  const [tab, setTab] = React20.useState("connection");
  const [applicationChoice, setApplicationChoice] = React20.useState("");
  const commitStatus = React20.useCallback((next) => {
  ...
  const beginConnect = React20.useCallback(async (force) => {
    setBusy("connect");
    try {
...
function ChannelLogo({ channel: channel4 }) {
  const definition = connectionDefinition(channel4);
  return h2("span", {
    className: `dim-logo ${definition.logoClass}`,
    "aria-hidden": "true"
  }, h2(definition.LogoGlyph));
}
function ChannelNavButton({ channel: channel4, activeId, onSelect }) {
  return h2(
    "button",
    {
      type: "button",
      ...
      "aria-selected": channel4.id === activeId,
      "aria-controls": `dim-panel-${channel4.id}`,
      onClick: () => onSelect(channel4.id)
    },
...
function IMSettingsTab(props) {
  ...
  const [activeGroupId, setActiveGroupId] = React21.useState(CHANNEL_GROUPS[0].id);
  const [loopbackRecovery, setLoopbackRecovery] = React21.useState(null);
  const githubTooltipId = React21.useId();
  const groupTabsId = React21.useId();
  const groupTabRefs = React21.useRef([]);
  ...
  const selectGroup = React21.useCallback((group) => {
    setActiveGroupId(group.id);
    setSelected((current) => group.channels.some((channel4) => channel4.id === current) ? current : group.channels[0].id);
  }, []);
  const onGroupTabKeyDown = React21.useCallback((event, index) => {
    let nextIndex;
        ...
        h2(
          ...
          h2(
            ...
            h2(
              ...
              activeGroup.channels.map((channel4) => h2(ChannelNavButton, {
                key: channel4.id,
                channel: channel4,
                activeId: active.id,
