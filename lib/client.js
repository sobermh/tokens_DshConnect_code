...
function BotCard({
  ...
  const { bot, health, state, connected: connected2 } = connection;
  ...
  const summary = actionError?.message ?? connection.error?.message ?? (connected2 ? null : health.summary);
...
function isFeishuPersonalStatusCacheFresh(now = Date.now()) {
  return statusCache !== void 0 && now - statusCachedAt < FEISHU_PERSONAL_STATUS_CACHE_TTL_MS;
}
function preloadFeishuPersonalStatus(rpcCall, { refresh = false } = {}) {
  ...
  const request = fetchFeishuPersonalStatus(rpcCall).then((status) => {
    if (statusCacheRevision !== revision && statusCache !== void 0) return statusCache;
...
function FeishuPersonalSettings({ rpcCall }) {
  ...
  const [status, setStatus] = React19.useState(initialStatus);
  const [error, setError] = React19.useState();
  const [busy, setBusy] = React19.useState(initialStatus === void 0 ? "load" : void 0);
  const [domain, setDomain] = React19.useState("feishu");
  const [tab, setTab] = React19.useState("connection");
  const [applicationChoice, setApplicationChoice] = React19.useState("");
  const commitStatus = React19.useCallback((next) => {
  ...
  const beginConnect = React19.useCallback(async (force) => {
    setBusy("connect");
    try {
...
async function fetchDingtalkPersonalStatus(rpcCall, signal) {
  return unwrap2(await rpcCall(DINGTALK_PERSONAL_ENDPOINTS.status, {}, signal));
}
async function connectDingtalkPersonal(rpcCall, { force = false } = {}, signal) {
  return unwrap2(await rpcCall(DINGTALK_PERSONAL_ENDPOINTS.connect, { force }, signal));
}
...
function isDingtalkPersonalStatusCacheFresh(now = Date.now()) {
  return statusCache2 !== void 0 && now - statusCachedAt2 <= DINGTALK_PERSONAL_STATUS_CACHE_TTL_MS;
}
function preloadDingtalkPersonalStatus(rpcCall, { refresh = false } = {}) {
  if (!refresh && isDingtalkPersonalStatusCacheFresh()) return Promise.resolve(statusCache2);
...
function HealthItem2({ active, title, detail }) {
  return h2(
    "div",
...
function ChannelLogo({ channel: channel4 }) {
  ...
  if (channel4 === "feishu-bot" || channel4 === "feishu-personal") return h2(FeishuLogo);
  if (channel4 === "dingtalk" || channel4 === "dingtalk-personal") return h2(DingtalkLogo);
...
function apply(ctx) {
  ...
  const feishuPersonalRpcCall = (endpoint, payload, signal) => ctx.connection.rpc.call(FEISHU_PERSONAL_RPC_CHANNEL, endpoint, payload, signal);
  const dingtalkPersonalRpcCall = (endpoint, payload, signal) => ctx.connection.rpc.call(DINGTALK_PERSONAL_RPC_CHANNEL, endpoint, payload, signal);
